# Run from the app/ directory:  py -m db.seed.script
import json
import os
import random
...
def seed_users():
    ...
    db.commit()
    print(f"  OK  {len(user_ids)} users seeded.")
    return user_ids
...
def seed_instructors():
    ...
    for inst in INSTRUCTORS:
        ...
        row = result.fetchone()
        if row:
            instructor_ids.append(row[0])

    db.commit()
    print(f"  OK  {len(instructor_ids)} instructors seeded.")
...
def seed_courses(instructor_ids):
    ...
    db.commit()
    print(f"  OK  {len(course_ids)} courses seeded.")
    return course_ids
...
def seed_comments(user_ids, instructor_ids, course_ids):
    ...
    for inst_id in instructor_ids:
        ...
        reviewers = random.sample(user_ids, min(num_comments, len(user_ids)))
    ...
    db.commit()
    print(f"  OK  {len(comment_ids)} comments seeded.")
    return comment_ids
...
def seed_replies(user_ids, comment_ids):
    ...
    for comment_id in comment_ids:
        if random.random() < 0.6:
            repliers = random.sample(user_ids, random.randint(1, 3))
    ...
    db.commit()
    print(f"  OK  {reply_count} replies seeded.")


def seed_reports(user_ids):
    print("Seeding reports...")

    comment_ids = [r[0] for r in db.execute(text("SELECT id FROM comments")).fetchall()]
    reply_ids   = [r[0] for r in db.execute(text("SELECT id FROM replies")).fetchall()]
    report_count = 0

    for comment_id in random.sample(comment_ids, min(len(comment_ids), random.randint(8, 18))):
        db.execute(
            text("""
                INSERT INTO reports (id, user_id, target_type, comment_id, reply_id, reason, description, is_reviewed)
